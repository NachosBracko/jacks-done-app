...
export default function App() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      setError(null)
      const [t, p, e, tl, ml] = await Promise.all([
      ...
      for (const r of [t, p, e, tl, ml]) { if (r.error) throw r.error }
      const summaries = (p.data || []).map(proj =>
        computeSummary(proj,
          (e.data || []).filter(x => x.project_id === proj.id),
          (tl.data || []).filter(x => x.project_id === proj.id),
          (ml.data || []).filter(x => x.project_id === proj.id)
        )
      )
      setTasks(t.data || [])
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col max-w-lg mx-auto">
      ...
      <main className="flex-1 overflow-y-auto px-4 pb-28">
        {loading ? (
          ...
          <>
            ...
            {view === 'projects' && <ProjectsView projects={projects} onRefresh={fetchAll} />}
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-lg mx-auto bg-gray-900 border-t border-gray-800 flex">
        {navItems.map(({ id, label, Icon }) => (
          <button key={id} onClick={() => setView(id)}
            className={`flex-1 flex flex-col items-center py-3 gap-1 transition-colors ${view === id ? 'text-blue-400' : 'text-gray-500'}`}>
            <Icon size={22} />
        ...
        <div className="fixed inset-0 bg-black/70 flex items-end z-50" onClick={() => setShowAddPicker(false)}>
          <div className="bg-gray-900 rounded-t-3xl w-full max-w-lg mx-auto p-6 pb-10" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                onClick={() => { setShowAddPicker(false); setShowAddTask(true) }}
              ...
              <button
                onClick={() => { setShowAddPicker(false); setShowAddProject(true) }}
      ...
      {showAddProject && <AddProjectForm onClose={() => setShowAddProject(false)} onSaved={() => { setShowAddProject(false); fetchAll() }} />}
    </div>
  )
}

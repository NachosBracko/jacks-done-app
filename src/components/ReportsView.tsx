...
export function ReportsView({ tasks, projects }: Props) {
  ...
  const inRange = (dateStr: string | null) => {
    ...
    try { return range === 'all' || isWithinInterval(parseISO(dateStr), monthInterval) }
    catch { return false }
  }
  ...
  const byType = useMemo(() => {
    const types: ProjectType[] = ['carflip', 'repair', 'mpflip']
    return types.map(type => {
      const ps = soldProjects.filter(p => p.project_type === type)
      const profit = ps.reduce((s, p) => s + (p.profit || 0), 0)
      const hours = ps.reduce((s, p) => s + p.total_hours, 0)
      const hourly = hours > 0 ? profit / hours : null
      return { type, count: ps.length, profit, hours, hourly }
    }).filter(t => t.count > 0)
  }, [soldProjects])
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gray-800/40 rounded-2xl p-4 border border-gray-800">
        <p className="text-gray-400 text-xs font-black uppercase tracking-wide mb-3">
          Overall — {soldProjects.length} completed
        </p>
      ...
      {byType.length > 0 && (
        <div className="bg-gray-800/40 rounded-2xl p-4 border border-gray-800">
          <p className="text-gray-400 text-xs font-black uppercase tracking-wide mb-3">By Category</p>
          <div className="space-y-3">
            {byType.map(t => (
              <div key={t.type} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="text-lg">{TYPE_EMOJI[t.type]}</span>
                  <div>
                    <p className="text-white font-semibold text-sm">{PROJECT_TYPE_LABEL[t.type]}</p>
                    <p className="text-gray-500 text-xs">{t.count} project{t.count !== 1 ? 's' : ''} · {t.hours.toFixed(1)}h</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className={`font-black text-sm ${t.profit >= 0 ? 'text-green-400' : 'text-red-400'}`}>{fmt(t.profit)}</p>
                  {t.hourly !== null && <p className="text-yellow-400 text-xs font-bold">{fmt(t.hourly)}/hr</p>}
                </div>
              </div>
            ))}
          </div>
      ...
      {activeInvested > 0 && (
        <div className="bg-gray-800/40 border border-gray-700 rounded-2xl p-4 flex items-center justify-between">
          <div>
            <p className="text-gray-300 text-sm font-bold">Tied up in active projects</p>
      ...
      {soldProjects.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-2">
            <p className="text-gray-400 text-xs font-black uppercase tracking-wide">Projects</p>
              ...
              <div key={p.id} className="bg-gray-800/30 rounded-xl p-3 border border-gray-800">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-2 min-w-0">
                    <span>{TYPE_EMOJI[p.project_type]}</span>
                    <p className="text-white font-semibold text-sm truncate">{p.title}</p>
                  </div>
                  <div className="flex items-center gap-1 shrink-0 ml-2">
                    {(p.profit||0) >= 0 ? <TrendingUp size={13} className="text-green-400" /> : <TrendingDown size={13} className="text-red-400" />}
          ...
          <div className="space-y-2">
            {tasksByCategory.map(([cat, count]) => (
              <div key={cat} className="flex items-center gap-2 text-sm">
                <span className="text-gray-400 w-20 shrink-0">{CATEGORY_LABEL[cat as keyof typeof CATEGORY_LABEL] || cat}</span>

...
export function AddProjectForm({ onClose, onSaved }: Props) {
  ...
  const handleSave = async () => {
    ...
    if (type === 'carflip') {
      ...
      make_val = make.trim()
      model_val = model.trim() || null
      year_val = year ? Number(year) : null
        ...
        <div className="space-y-4">
          ...
          {type === 'repair' && (
            <div>
              ...
              <input autoFocus value={customerName} onChange={e => setCustomerName(e.target.value)} placeholder="e.g. Tony Smith"
                className="w-full bg-gray-800 text-white rounded-xl px-4 py-3 text-base outline-none focus:ring-2 focus:ring-green-500" />
            </div>
          )}
          ...
          {/* Cost + notes for repair and mpflip */}
          {(type === 'repair' || type === 'mpflip') && (
            <div>
              <label className="text-gray-400 text-sm mb-1 block">Starting cost (parts, purchase, etc.)</label>
              <input type="number" value={purchasePrice} onChange={e => setPurchasePrice(e.target.value)} placeholder="$0"
                className="w-full bg-gray-800 text-white rounded-xl px-4 py-3 text-base outline-none focus:ring-2 focus:ring-green-500" />
            </div>
          )}

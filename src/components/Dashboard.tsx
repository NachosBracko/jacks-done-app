...
const CATEGORY_COLOR: Record<Category, string> = {
  ...
  repairs:   'bg-blue-500/20 border-blue-500/40',
  carflips:  'bg-orange-500/20 border-orange-500/40',
  mpflips:   'bg-yellow-500/20 border-yellow-500/40',
...
const CATEGORY_DOT: Record<Category, string> = {
  ...
  repairs:   'bg-blue-400',
  carflips:  'bg-orange-400',
  mpflips:   'bg-yellow-400',
  other:     'bg-gray-400',
}
        ...
        {tasks.map(task => {
          ...
          return (
            <div key={task.id} className={`rounded-2xl border px-4 py-3 flex items-center gap-3 ${CATEGORY_COLOR[task.category]}`}>
              ...
              <button onClick={() => onComplete(task.id)} className="text-gray-500 active:text-green-400 transition-colors shrink-0">
